...
type Attack struct {
	...
	End			int64
	Created 		int64
	ServerIP   string // 新增 ServerIP 字段
	ServerPort int    // 新增 ServerPort 字段 
	// Branch       string // 添加 Branch 字段
}
...
func LogAttack(Attacks *Attack) (bool, error) {
	_, error := Database.Query("INSERT INTO `attacks` (`ID`, `Username`, `Target`, `Method`, `Port`, `Duration`, `End`, `Created`, `ServerIP`, `ServerPort`) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?)", // 添加 ServerIP 和 ServerPort 字段
		Attacks.Username,
		Attacks.Target,
		Attacks.Method,
		Attacks.Port,
		...
		Attacks.End,
		Attacks.Created,
		// Attacks.Branch, // 在这里添加 Branch 字段的值
		Attacks.ServerIP, // 传入 ServerIP 参数
		Attacks.ServerPort, // 传入 ServerPort 参数
	); if error != nil {
		return false, error
	}
	return true, nil
...
func Ongoing() ([]*Attack, error) {
	...
	rows, err = Database.Query("SELECT `ID`, `Username`, `Target`, `Method`, `Port`, `Duration`, `End`, `Created`, `ServerIP`, `ServerPort` FROM `attacks` WHERE `End` > ?", time.Now().Unix()) // 添加 ServerIP 和 ServerPort
...
func MyAttacking(User string) ([]*Attack, error) {
	...
	rows, err = Database.Query("SELECT `ID`, `Username`, `Target`, `Method`, `Port`, `Duration`, `End`, `Created`, `ServerIP`, `ServerPort` FROM `attacks` WHERE `End` > ? AND `username` = ?", time.Now().Unix(), User) // 添加 ServerIP 和 ServerPort
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
...
func scanOngoing(row *sql.Rows, Attacking *Attack) error {
	...
	return row.Scan(
		...
		&Attacking.End, 
		&Attacking.Created, 
		&Attacking.ServerIP, //  添加 ServerIP
        &Attacking.ServerPort, // 添加 ServerPort
	)
}