...
func AddTime(user string, TimeUnit time.Duration) bool {
	...
	// Time_Asending := Plan_time.Sub(TimeUnit).Unix()
...
func SetExpiryTime(user string, newExpiryTime time.Time) bool {
	Userss, error := GetUser(user)
	if error != nil {
		return false
	}

	// 直接设置新的到期时间
	Time_Asending := newExpiryTime.Unix()

	Row, error := Database.Query("UPDATE `users` SET `PlanExpiry` = ? WHERE `username` = ?", Time_Asending, Userss)
	if error != nil || Row.Err() != nil {
		return false
	}
...
//2024
func EditFeildadd(user, feild, replace string) bool {
	// 使用 `+=` 操作符进行加法运算
	sql := fmt.Sprintf("update `users` set %s = %s + ? where username = ?", feild, feild)

	Row, error := Database.Query(sql, replace, user)
	if error != nil || Row.Err() != nil {
		log.Println(error)
		return false
	}
