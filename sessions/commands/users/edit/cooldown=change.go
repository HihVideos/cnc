...
func CooldownTimeChange(session *sessions.Session_Store, cmd []string, stringsep []string) {
	...
	if !strings.Contains(strings.Replace(strings.Join(stringsep, "="), stringsep[1], "", -1), "addtime=") || len(cmd) <= 2 {
	...
	for LenCon := 2; LenCon < len(cmd); LenCon++ {
		...
		//EditFeildadd 是增加，EditFeild是设置
		boolen_error := database.EditFeildadd(cmd[LenCon], "Cooldown", stringsep[1])
		if !boolen_error {
		...
		for _, Session := range sessions.SessionMap {
			if Session.User.Username == cmd[LenCon] {
				Cooldown, err := strconv.Atoi(stringsep[1]) // 转换 Cooldown 为 int 类型
				if err != nil {
					// 处理转换错误
					return
				}
				NewCooldown := Session.User.Cooldown + Cooldown
				Session.Channel.Write([]byte("\x1b[0m\x1b7\x1b[1A\r\x1b[2K \x1b[38;5;105m你的攻击时间从 "+strconv.Itoa(User.Cooldown)+" 更改到 "+strconv.Itoa(NewCooldown)+"\x1b[0m\x1b[38;5;15m\x1b8"))
				Session.User.Cooldown = NewCooldown
			}
		}
...
func SetTime(session *sessions.Session_Store, cmd []string, stringsep []string) {

	if !strings.Contains(strings.Replace(strings.Join(stringsep, "="), stringsep[1], "", -1), "settime=") || len(cmd) <= 2 {
		session.Channel.Write([]byte("\x1b[0m"+build.Config.AppConfig.AppName+" -> Command Example: users cooldown=<New cooldown> [users array, eg \"root root123456\"]\r\n"))
		return
	}
